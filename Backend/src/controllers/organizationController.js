...
const listSchema = Joi.object({
  q: Joi.string(),
  type: Joi.string().valid('company', 'agency'),
  industry: Joi.string(),
  size: Joi.string(),
  location: Joi.string(),
  tags: commaSeparated,
...
const list = async (req, res, next) => {
  try {
    ...
    const result = await marketplaceService.listOrganizations(payload, req.user);
    res.json(result);
  } catch (error) {
    next(error);
  }
...
const get = async (req, res, next) => {
  try {
    ...
    const organization = await marketplaceService.getOrganization(req.params.id, options, req.user);
...
const create = async (req, res, next) => {
  try {
    ...
    const options = buildViewOptions(req);
    const organization = await marketplaceService.createOrganization(payload, req.user, options);
    const body = { data: organization };
...
const update = async (req, res, next) => {
  try {
    ...
    const options = buildViewOptions(req);
    const organization = await marketplaceService.updateOrganization(req.params.id, payload, req.user, options);
    const body = { data: organization };
...
const remove = async (req, res, next) => {
  try {
    const result = await marketplaceService.deleteOrganization(req.params.id, req.user);
...
const analytics = async (req, res, next) => {
  try {
    const payload = buildListPayload(req);
    const data = await marketplaceService.organizationAnalytics(payload);
    res.json({ data });
  } catch (error) {
    next(error);
  }
